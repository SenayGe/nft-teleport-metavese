...
interface IL1ERC721 is IERC721{
    ...
    function transferToken (address _from, address _to, uint256 _tokenId, bytes _data) external;
...
contract BridgeEth is FxBaseRootTunnel, IERC721Receiver{
    ...
    */ 
    function deposit(
        ...
        string memory tokenURI = IL1ERC721(originalToken).tokenURI(tokenId);
        bytes memory bridgeMessage = abi.encode(DEPOSIT, abi.encode(rootToken, msg.sender, user, tokenId, tokenURI));
        _sendMessageToChild(bridgeMessage);
        emit DepositNFT(originalToken, msg.sender, user, tokenId);
